...
function snax_admin_settings_fields_polls( $fields ) {
	$fields['snax_settings_polls'] = array(
		...
		'snax_poll_allow_guests_to_play' => array(
			'title'             => __( 'Allow guests to play', 'snax' ),
			'callback'          => 'snax_admin_setting_poll_allow_guests_to_play',
			'sanitize_callback' => 'sanitize_text_field',
			'args'              => array(),
		),
...
function snax_admin_polls_settings() {
	...
	<div class="wrap">
		...
		<h2 class="nav-tab-wrapper"><?php snax_admin_settings_tabs( __( 'Polls', 'snax' ) ); ?></h2>
 ...
 * Featured media on single post
 */
function snax_admin_setting_poll_allow_guests_to_play() {
	$checked = snax_poll_allow_guests_to_play();
	?>
	<input name="snax_poll_allow_guests_to_play" id="snax_poll_allow_guests_to_play" value="standard" type="checkbox" <?php checked( $checked ); ?> />
	<?php
}
